...
# network_local.load_weights("../../train/train_1mins/out/savemodel/target/RHfile_3400")
...
env = gymRoom.roomex(physicalTimeStep=60)
...
FanPowerSUM = sum(each_FanPower_history)

print('score:', episode_reward, "FanPowerSUM:", FanPowerSUM)

# 保存episode_reward_history和each_episode_rewards_history数据

name = [str(episode_count)]
# print([episode_reward_history])
pd.DataFrame(data=[episode_reward_history], index=name).to_csv(episode_reward_history_csv, sep=',',
                                                               encoding='utf-8')
pd.DataFrame(data=[each_episode_rewards_history], index=name).to_csv(each_episode_rewards_history_csv,
                                                                     sep=',', mode='a', encoding='utf-8')
pd.DataFrame(data=[each_FanPower_history], index=name).to_csv(each_FanPower_history_csv, sep=',', mode='a',
                                                              encoding='utf-8')
pd.DataFrame(data=[FanPower_history], index=name).to_csv(FanPower_history_csv, sep=',', encoding='utf-8')

# 把每一轮的 RHfile.csv 历史另存为 RHfile_episode_count.csv
dstFile = all_log_path[1] + 'RHfile_' + str(episode_count) + '.csv'
try:
...
# 判断模型收敛并结束训练的条件
if len(episode_reward_history) >= 10 and (np.mean(episode_reward_history[-10:])) > -50:
