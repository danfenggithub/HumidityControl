...
class DQNAgent:
    ...
    def act(self, state):
        ...
        if random.random() > eps:
            ...
            HRRandom = random.choice(a_action)
            action = [40, 40, 40,
                      random.choice(b_action), random.choice(d_action), random.choice(f_action)]
            return action
    ...
    def learn(self, transitions):
        ...
        with tf.GradientTape() as tape:
            ...
            # with dqn_rainbow.summary_writer.as_default():  # 指定记录器
            #     tf.summary.scalar("loss", loss, step=self.t_step)  # 将当前损失函数的值写入记录器
            # get gradients
            gradients = tape.gradient(loss, self.network_local.trainable_weights)
