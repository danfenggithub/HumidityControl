...
RHRewardList = [IfElseRHRewardList / 15, Power1RHRewardList / 15, Power2RHRewardList / 15,
                Power3RHRewardList / 15, points6RHRewardList / 15]
...
plt.tight_layout()
plt.savefig(savepic + "不同策略的平均绝对湿度差变化曲线.png")
# plt.show()
...
ax2.set_xlabel('单位为30秒的时间片', fontsize=20)
ax2.set_ylabel('恒湿机功率（kw/h）', fontsize=20)
plt.xticks(fontproperties='Times New Roman', size=20)
plt.yticks(fontproperties='Times New Roman', size=20)
plt.legend(loc="upper right", fontsize=20)
plt.tight_layout()
plt.savefig(savepic + "不同策略的功率变化曲线.png")
# plt.show()
...
plt.yticks(fontproperties='Times New Roman', size=20)
plt.tight_layout()
plt.savefig(savepic + "不同策略的总功耗对比.png")
# plt.show()
